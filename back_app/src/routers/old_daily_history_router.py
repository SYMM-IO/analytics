...
from src.app.subgraph_models import DailyHistory, TotalHistory
...
async def get_total_history():
    total_history = AttrDict(dict(deposits=Decimal(0), trade_volume=Decimal(0), quotes=Decimal(0), users=Decimal(0), accounts=Decimal(0)))
    model = TotalHistory
    ...
    for context in contexts + unsync_contexts:
        ...
        condition = [GraphQlCondition(field="collateral", operator=None, value=f'"{context.symmio_collateral_address}"')]
        load_params_map[model] = LoadParams(fields=fields, conditions=condition)
        out = subgraph_client.load_all(
            load_params_map=load_params_map, create_function=lambda model, data: subgraph_client.create_function(model, data)
        )
        for res in out:
            for i in res:
