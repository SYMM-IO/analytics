...
from src.config import arbitrum_8_2_contexts
from src.config import base_8_2_contexts
from src.config import blast_8_2_contexts
from src.config import bnb_8_2_contexts
from src.config import mantle_8_2_contexts
...
Multi account""".split("\n")
contexts = dict(
    ARBITRUM_8_2=arbitrum_8_2_contexts,
    BASE_8_2=base_8_2_contexts,
    BLAST_8_2=blast_8_2_contexts,
    BNB_8_2=bnb_8_2_contexts,
    MANTLE_8_2=mantle_8_2_contexts,
)
affiliates = dict()
...
for chain in contexts:
    ...
    with db_session() as session:
        account: List[Account] = session.scalars(select(Account).options(load_only(Account.accountSource, Account.id, Account.timestamp)))
        for acc in account:
...
for affiliate in set(affiliates.values()):
    ...
    with db_session() as session:
        quotes: List[Quote] = session.scalars(
            select(Quote)
            .where(Quote.account_id.in_(valid_account_ids))
            .options(
                load_only(
                    Quote.tenant,
                    Quote.quoteStatus,
        ...
        for quote in quotes:
            ...
            df_dict["Platform fees paid"].append(
                (Decimal(quote.initialOpenedPrice or 0) / DEC18) * (Decimal(quote.quantity) / DEC18) * (Decimal(quote.tradingFee) / DEC18)
            )
            df_dict["Locked collateral"].append(Decimal(quote.initialCva + quote.initialLf) / DEC18)
            df_dict["Leverage"].append(
                ((Decimal(quote.initialOpenedPrice or 0) * Decimal(quote.quantity or 0)) / Decimal(quote.initialCva + quote.initialLf)) / DEC18
            )
            ...
            df_dict["Open size"].append((Decimal(quote.initialOpenedPrice or 0) / DEC18) * (Decimal(quote.quantity) / DEC18))
            df_dict["Close size"].append((Decimal(quote.averageClosedPrice) / DEC18) * (Decimal(quote.closedAmount) / DEC18))
            ...
            if quote.quoteStatus == 7 and quote.averageClosedPrice:
                df_dict["Price change (%)"].append(100 * (quote.averageClosedPrice - quote.initialOpenedPrice) / quote.initialOpenedPrice)
            elif quote.quoteStatus == 8 and quote.liquidatePrice:
                df_dict["Price change (%)"].append(100 * (quote.liquidatePrice - quote.initialOpenedPrice) / quote.initialOpenedPrice)
            else:
                df_dict["Price change (%)"].append(None)
            pl_usdGross_realized = (
                (1 if quote.positionType == 0 else -1)
                * ((Decimal(quote.averageClosedPrice or 0) - Decimal(quote.initialOpenedPrice or 0)) / DEC18)
                * (Decimal(quote.quantity) / DEC18)
                - Decimal(quote.userPaidFunding) / DEC18
                - (Decimal(quote.initialOpenedPrice or 0) / DEC18) * (Decimal(quote.quantity) / DEC18) * (Decimal(quote.tradingFee) / DEC18)
            )
            df_dict["Profit"].append(pl_usdGross_realized)
            df_dict["Return"].append(100 * pl_usdGross_realized / (Decimal(quote.initialLf + quote.initialCva + quote.initialPartyAmm) / DEC18))
