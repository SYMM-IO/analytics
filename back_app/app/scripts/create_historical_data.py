...
async def prepare_historical_snapshots():
    ...
    with db_session() as session:
        transaction = LogTransaction(start_time=datetime.now(),
                                     label=f'{context.tenant} {prepare_historical_snapshots.__name__}')
        ...
        while snapshot_block.timestamp() > begin_timestamp:
            with span_context(session, prepare_historical_snapshots.__name__, transaction.id) as span:
