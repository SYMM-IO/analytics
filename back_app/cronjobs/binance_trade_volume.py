...
def fetch_and_save_all_trades(
    ...
    while True:
        ...
        print(
            f"{context.tenant}: Loading binance trades between: start_time={start_time}, end_time={end_time}"
        )
        current_trades = hedger_context.utils.binance_client.futures_account_trades(
            limit=1000, startTime=int(start_time), endTime=int(end_time)
        )
