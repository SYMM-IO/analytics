...
export class ChartComponent implements OnInit, OnDestroy {
	...
	private subscribeToDataChanges() {
		const subscription = this.dailyAffiliateHistories.subscribe((results) => {
	...
	private setupChartEventListeners() {
		this.chart.on('legendselectchanged', (params: any) => {
			this.visibleSeries = this.chart.getOption().series
				.filter((series: any) => params.selected[series.name])
	...
	private updateChart(affiliateHistories: AffiliateHistory[], fieldName: string) {
		const series = this.prepareSeries(affiliateHistories, fieldName);
	...
	private prepareSeries(affiliateHistories: AffiliateHistory[], fieldName: string) {
		const series = [];
		...
		for (const affiliateHistory of affiliateHistories) {
			...
			const preparedResults = this.prepareResults(this.getHistoryByView(affiliateHistory, view!), fieldName);
			series.push(this.createSeriesItem(affiliateHistory, preparedResults, fieldName, view!));
		}
	...
	private getHistoryByView(affiliateHistory: AffiliateHistory, view: string) {
		switch (view) {
			case 'Daily':
				return affiliateHistory.histories;
			case 'Weekly':
				return affiliateHistory.weeklyHistories;
			case 'Monthly':
				return affiliateHistory.monthlyHistories;
			default:
				return affiliateHistory.histories;
		}
	}

	private createSeriesItem(affiliateHistory: AffiliateHistory, preparedResults: any, fieldName: string, view: string) {
		return {
			type: "bar",
			stack: 'total',
			color: affiliateHistory.affiliate.mainColor,
			name: this.fixedValueName || affiliateHistory.affiliate.name,
			data: preparedResults.data.map((history: any) => [
				new Date(this.getTimeByView(history, view)),
				(history[fieldName] as BigNumber).div(BigNumber(10).pow(this.decimals)).toNumber(),
			]),
			animation: true,
		};
	}
	...
	private prepareResults(histories: any[], fieldName: string) {
		let data = histories.filter(history => {
